...
def test_resize(img, size=640, pad=False):
    ...
    if pad:
        ...
        new_img[:h, :w] = cv2.resize(img, (size, size))
    else:
...
def draw_bbox(img, result, color=(255, 0, 0), thickness=1, bbox=False):
    """
    :input: RGB img
    """
...
def get_box_preds(args, img_fn, origin_info, batch, preds, vis_char=False):
...
def visualize_polygon(args, img_fn, origin_info, batch, preds, vis_char=False):
    ...
    tmp_img = draw_bbox(img_origin, np.array(box_list, dtype='object'))
