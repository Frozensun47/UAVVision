...
class BaseDatasetIter(Dataset):
    def __init__(self,
        ...
        self.image_paths, self.gt_paths = self.load_metadata(
            self.train_dir, self.train_gt_dir)
        # load annotation
        self.all_anns = self.load_all_anns(self.gt_paths)
        print(len(self.image_paths), len(self.gt_paths), len(self.all_anns))
        print(np.shape(self.all_anns))
    ...
    def __getitem__(self, index):
        ...
        data_return = {
            ...
            # "class_0_map": class_0_map,
            # "classs_1_map":class_1_map
        }
        # for batch_size = 1
    ...
    def load_all_anns(self, gt_paths):
        res = []
        for gt in gt_paths:
            mat_data = scipy.io.loadmat(gt)
            instances = mat_data['polygt']
            all_points = []
            for instance in instances:
                item = {}
                x_values = instance[1][0]
                y_values = instance[3][0]
                try:
                    label = instance[4][0]
                except:
                    label = self.ignore_tags[0]
                points = np.array(np.column_stack(
                    (x_values, y_values))).reshape((-1, 2)).tolist()
                if len(points) < 3:
                    continue
                item['poly'] = points
                item['text'] = label
