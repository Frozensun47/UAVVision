...
def draw_thresh_map(polygon, canvas, mask, shrink_ratio=0.4):
    ...
    # print('canvas', np.shape(
    #     canvas[ymin_valid:ymax_valid + 1, xmin_valid:xmax_valid + 1]))
    # print('dm', np.shape(distance_map[ymin_valid - ymin:ymax_valid - ymax + height,
    #                                   xmin_valid - xmin:xmax_valid - xmax + width]),)
    

    # canvas[ymin_valid:ymax_valid + 1, xmin_valid:xmax_valid + 1] = np.fmax(
    #     np.ones_like(distance_map[ymin_valid - ymin:ymax_valid - ymax + height,
    #                               xmin_valid - xmin:xmax_valid - xmax + width], dtype=np.float32)
    #     - distance_map[ymin_valid - ymin:ymax_valid - ymax + height,
    #                      xmin_valid - xmin:xmax_valid - xmax + width],
    #     canvas[ymin_valid:ymax_valid + 1, xmin_valid:xmax_valid + 1])
...
def compute_distance(xs, ys, point_1, point_2):
    ...
    square_distance = np.square(point_1[0] -
                                point_2[0]) + np.square(point_1[1] -
                                                        point_2[1])
    square_distance_1 = np.where(square_distance_1 == 0, 1e-15, square_distance_1)
    square_distance_2 = np.where(square_distance_2 == 0, 1e-15, square_distance_2)
