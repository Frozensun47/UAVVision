...
def main(net, args):
    ...
    print('pred ',preds.to('cpu')[0].numpy().shape)
    if args.heatmap:
        ...
        visualize_polygon(args, img_fn, (img_origin, h_origin, w_origin),
                          batch, preds)
    plt.imshow(preds.to('cpu')[0].numpy()[0])
    plt.axis('off')
    img_fn_pred = "pred_result_0_{}".format(img_fn)
    plt.savefig(os.path.join(args.save_dir, img_fn_pred),
                dpi=300,
                bbox_inches='tight')
    plt.close()
    plt.imshow(preds.to('cpu')[0].numpy()[1])
    plt.axis('off')
    img_fn_pred = "pred_result_1_{}".format(img_fn)
    plt.savefig(os.path.join(args.save_dir, img_fn_pred),
                dpi=300,
                bbox_inches='tight')
    binary_pred =(preds.to('cpu')[0].numpy()[0] > 0.5).astype(np.uint8)
    plt.imshow(binary_pred,cmap='gray')  # , cmap='gray'
    plt.axis('off')
    img_fn_pred = "binary_pred_result_bw_{}".format(img_fn)
    plt.savefig(os.path.join(args.save_dir, img_fn_pred),
